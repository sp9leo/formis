...
frappe.provide("erpnext.formisbom");

frappe.ui.form.on("FormisBOM", {
		...
		if (!frm.doc.__islocal && frm.doc.docstatus<2) {
			...
			frm.add_custom_button(__("Browse BOM"), function() {
				frappe.route_options = {
					"FormisBOM": frm.doc.name
				};
				frappe.set_route("Tree", "FormisBOM");
			});
		}
...
cur_frm.cscript.hour_rate = function(doc, dt, dn) {
	erpnext.formisbom.calculate_op_cost(doc);
	erpnext.formisbom.calculate_total(doc);
}
...
var get_bom_material_detail= function(doc, cdt, cdn) {
	...
	if (d.item_code) {
		return frappe.call({
			doc: doc,
			method: "get_formisbom_material_detail",
			args: {
				'item_code': d.item_code,
				'formisbom_no': d.formisbom_no != null ? d.formisbom_no: '',
				'qty': d.qty
			},
			callback: function(r) {
				d = locals[cdt][cdn];
				...
				erpnext.formisbom.calculate_rm_cost(doc);
				erpnext.formisbom.calculate_total(doc);
			},
			freeze: true
		});
...
cur_frm.cscript.rate = function(doc, cdt, cdn) {
	...
	if (d.formisbom_no) {
		...
		get_formisbom_material_detail(doc, cdt, cdn);
	} else {
		erpnext.formisbom.calculate_rm_cost(doc);
		erpnext.formisbom.calculate_total(doc);
	}
...
erpnext.formisbom.calculate_op_cost = function(doc) {
...
erpnext.formisbom.calculate_rm_cost = function(doc) {
	var rm = doc.items || [];
	total_rm_cost = 0;
...
erpnext.formisbom.calculate_total = function(doc) {
...
cur_frm.fields_dict['items'].grid.get_field('formisbom_no').get_query = function(doc, cdt, cdn) {
	var d = locals[cdt][cdn];
	return{
		filters:{
			'item': d.item_code,
...
frappe.ui.form.on("BOM Operation", "workstation", function(frm, cdt, cdn) {
	...
	frappe.call({
		...
		callback: function (data) {
			...
			erpnext.formisbom.calculate_op_cost(frm.doc);
			erpnext.formisbom.calculate_total(frm.doc);
		}
	})
});
...
frappe.ui.form.on("BOM Item", "items_remove", function(frm) {
	erpnext.formisbom.calculate_rm_cost(frm.doc);
	erpnext.formisbom.calculate_total(frm.doc);
});
...
frappe.ui.form.on("FormisBOM", "with_operations", function(frm) {
