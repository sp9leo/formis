...
frappe.ui.form.on("calc", "refresh", function(frm) {
...
frappe.ui.form.on("calc", "button", function(frm) {
frm.refresh();
});
...
frappe.ui.form.on("calc", "doc_1", function(frm) {
cur_frm.set_value("price1", (frm.doc.price1)+" za "+(frm.doc.item_uom));

 });
//test html-ja
frappe.ui.form.on("calc", "refresh", function(frm,doctype,name) {
 ...
 //izracunaj povrsino//
 frappe.ui.form.on("calc", "x", function(frm) {
 cur_frm.set_value("area", (frm.doc.x*frm.doc.y)/10000);
    
 });
frappe.ui.form.on("calc", "y", function(frm) {
 cur_frm.set_value("area", (frm.doc.x*frm.doc.y)/10000);
...
frappe.ui.form.on("calc", "man_price", function(frm) {
